...
export default class command extends BaseCommand {
    override execute = async (M: Message): Promise<void> => {
        ...
        let text = `ðŸ€ *Question: ${quizzes[0].question}*\n`
        ...
        setTimeout(async () => {
            ...
            if (this.handler.quiz.game.has(M.from)) {
                ...
                await this.client.sendMessage(M.from, {
                    text: `ðŸ•• Timed out! The correct answer was ${
        ...
        const id = setInterval(async () => {
            ...
            if (counter > quizzes.length - 1) {
                ...
                if (length > 0 && (players?.players[0].points as number) > 0) {
                    ...
                    await this.client.DB.user.updateOne(
                        { jid: players?.players[0].jid as string },
                        { $inc: { quizWins: 1 } }
                    )
                    for (let i = 0; i < length; i++)
                    ...
                    Text += `\n\nðŸŽ‰ *@${players?.players[0].jid.split('@')[0]}* won this one ðŸŽ‰`
            ...
            let text = `ðŸ€ *Question: ${quizzes[counter].question}*\n`
    ...
    private emojis = ['1ï¸âƒ£', '2ï¸âƒ£', '3ï¸âƒ£', '4ï¸âƒ£']
}